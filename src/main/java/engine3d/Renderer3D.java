...
public class Renderer3D extends Renderer {
    ...
    private void setPixel(int x, int y, float depth, int color) {
        if ( depth > depthBuffer[y * getWidth() + x] ) {
            super.setPixel(x, y, color);
            depthBuffer[y * getWidth() + x] = depth;
        }
    }
    ...
    private void setPixelForTriangle(int y, int x, float w, float brightness, int color) {
        ...
        try {
            if ( w > depthBuffer[y * getWidth() + x] ) {
                setPixel(x, y, color);
                depthBuffer[y * getWidth() + x] = w;
            }
    ...
    private void setPixelForTexturedTriangle(int y, int x,
        ...
        try {
            if ( tex_w > depthBuffer[y * getWidth() + x] ) {
                setPixel(x, y, color);
                depthBuffer[y * getWidth() + x] = tex_w;
            }
        } catch ( ArrayIndexOutOfBoundsException e ) {
            String errorMessage = "X: " + x + " Y: " + y + " outside of " + getWidth() + "x" + getHeight();
            //System.out.println("Set pixel Error: " + errorMessage + e.getMessage());
